...
/** Remove the transaction record only — bank balance is NOT changed */
export const deleteTransactionOnly = async (id) => {
  const db = await initDB();
  await db.delete('transactions', Number(id));
};

/**
 * Remove the transaction AND reverse its effect on the bank balance:
 * - debit  → add the amount back (bank gains)
 * - credit → subtract the amount (bank loses)
 */
export const deleteTransactionAndRestore = async (id) => {
  const db = await initDB();
  const tx = await db.get('transactions', Number(id));
  if (!tx) throw new Error('Transaction not found');

  // Reverse the original balance change
  const reversal = tx.type === 'debit' ? Number(tx.amount) : -Number(tx.amount);
  await updateBankBalance(tx.bankId, reversal);
  await db.delete('transactions', Number(id));
};